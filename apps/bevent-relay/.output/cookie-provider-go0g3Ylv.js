import { i as __require, t as __commonJSMin } from "./chunk-CrysgU_F.js";
...
//#endregion
//#region ../../node_modules/.pnpm/dotenv@17.3.1/node_modules/dotenv/lib/env-options.js
...
//#endregion
//#region ../../node_modules/.pnpm/dotenv@17.3.1/node_modules/dotenv/lib/cli-options.js
...
//#endregion
//#region ../../node_modules/.pnpm/dotenv@17.3.1/node_modules/dotenv/config.js
(function() {
	require_main().config(Object.assign({}, require_env_options(), require_cli_options()(process.argv)));
})();
//#endregion
//#region ../../node_modules/.pnpm/@viyuni+cookie-crypto@0.0.1/node_modules/@viyuni/cookie-crypto/dist/index.js
const ALGORITHM = "aes-256-gcm";
const DEFAULT_IV_LENGTH = 12;
const PAYLOAD_VERSION = "v1";
var CookieCryptoError = class extends Error {
	constructor(message) {
		super(message);
		this.name = "CookieCryptoError";
	}
};
function deriveKey(secret) {
	if (!secret) throw new CookieCryptoError("Secret is required.");
...
function createIv(options) {
	if (options?.iv) {
		if (options.iv.length === 0) throw new CookieCryptoError("IV cannot be empty.");
		return Buffer.from(options.iv);
	}
	const ivLength = options?.ivLength ?? DEFAULT_IV_LENGTH;
	if (!Number.isInteger(ivLength) || ivLength < 12) throw new CookieCryptoError("ivLength must be an integer >= 12.");
	return randomBytes(ivLength);
}
function setAad(cipher, aad) {
	if (!aad) return;
	cipher.setAAD(Buffer.from(aad, "utf8"));
}
function encryptCookie(cookie, secret, options) {
	if (typeof cookie !== "string" || cookie.length === 0) throw new CookieCryptoError("Cookie must be a non-empty string.");
	const key = deriveKey(secret);
	const iv = createIv(options);
	try {
		const cipher = createCipheriv(ALGORITHM, key, iv);
		setAad(cipher, options?.aad);
		const encrypted = Buffer.concat([cipher.update(cookie, "utf8"), cipher.final()]);
		const tag = cipher.getAuthTag();
		return `${PAYLOAD_VERSION}.${toBase64Url(iv)}.${toBase64Url(encrypted)}.${toBase64Url(tag)}`;
	} finally {
		key.fill(0);
	}
}
function decryptCookie(payload, secret, options) {
	if (typeof payload !== "string" || payload.length === 0) throw new CookieCryptoError("Payload must be a non-empty string.");
	const parts = payload.split(".");
	if (parts.length !== 4) throw new CookieCryptoError("Invalid payload format.");
	const [version, ivBase64, cipherBase64, tagBase64] = parts;
	if (version !== PAYLOAD_VERSION) throw new CookieCryptoError(`Unsupported payload version: ${version}`);
	const key = deriveKey(secret);
	const iv = fromBase64Url(ivBase64);
	const ciphertext = fromBase64Url(cipherBase64);
	const tag = fromBase64Url(tagBase64);
	const decipher = createDecipheriv(ALGORITHM, key, iv);
	setAad(decipher, options?.aad);
	decipher.setAuthTag(tag);
	try {
		return Buffer.concat([decipher.update(ciphertext), decipher.final()]).toString("utf8");
	} catch {
		throw new CookieCryptoError("Decrypt failed. Secret, AAD, or payload may be invalid.");
	}
}
function createCookieCrypto(secret) {
	return {
		encrypt: (cookie, options) => encryptCookie(cookie, secret, options),
		decrypt: (payload, options) => decryptCookie(payload, secret, options)
	};
}
//#endregion
//#region ../../node_modules/.pnpm/@viyuni+cookie-sync-client@0.0.3/node_modules/@viyuni/cookie-sync-client/dist/index.js
var CookieSyncClientError = class extends Error {
	constructor(message) {
		super(message);
		this.name = "CookieSyncClientError";
	}
};
/**
* Expected connectionUrl format:
* `http://name:accessToken@localhost:3000`
* - `name` is used as the record key in `/:name`
* - `accessToken` is sent as Bearer token
*/
function parseConnectionUrl(connectionUrl) {
	...
	try {
		...
		throw new CookieSyncClientError("Invalid connectionUrl.");
	}
	if (!url.username) throw new CookieSyncClientError("connectionUrl must include name in username part.");
	if (!url.password) throw new CookieSyncClientError("connectionUrl must include accessToken in password part.");
	const name = decodeURIComponent(url.username);
	const accessToken = decodeURIComponent(url.password);
	const normalizedPath = url.pathname.endsWith("/") ? url.pathname : `${url.pathname}/`;
	const baseUrl = `${url.protocol}//${url.host}${normalizedPath}`;
	return {
		endpoint: new URL(`api/${encodeURIComponent(name)}`, baseUrl).toString(),
		accessToken
	};
}
async function parseResponseValue(response) {
...
function assertOk(response, action) {
	...
	throw new CookieSyncClientError(`${action} failed with status ${response.status}.`);
}
/**
* Creates a cookie sync client.
*
* @param connectionUrl - Must be in format `http://name:accessToken@localhost:3000`
* @param password - Secret used by `@viyuni/cookie-crypto` to encrypt/decrypt cookie
*/
function createCookieSyncClient(connectionUrl, password) {
	if (!password) throw new CookieSyncClientError("password is required.");
	const { endpoint, accessToken } = parseConnectionUrl(connectionUrl);
	const crypto = createCookieCrypto(password);
	function getAuthorization(accessToken$1) {
		return `Bearer ${accessToken$1}`;
	}
	async function sync(cookie) {
		if (!cookie) throw new CookieSyncClientError("cookie is required.");
		const encrypted = crypto.encrypt(cookie);
		assertOk(await fetch(endpoint, {
			method: "POST",
			headers: {
	...
	async function getDecodedCookie() {
		const response = await fetch(endpoint, {
			method: "GET",
			headers: { Authorization: getAuthorization(accessToken) }
		});
		assertOk(response, "Get cookie");
		const encrypted = await parseResponseValue(response);
...
//#endregion
//#region ../../node_modules/.pnpm/arkenv@0.11.0_arktype@2.2.0/node_modules/arkenv/dist/core-DzT5rjcY.mjs
...
`), t = {
	...
	cyan: `\x1B[36m`,
	reset: `\x1B[0m`
}, n = () => typeof process < `u` && process.versions != null && process.versions.node != null, r = () => !!(!n() || process.env.NO_COLOR !== void 0 || process.env.CI !== void 0 || process.stdout && !process.stdout.isTTY), i = (e, i) => n() && !r() ? `${t[e]}${i}${t.reset}` : i, a$1 = (e) => e.map((e) => `${i(`yellow`, e.path)} ${e.message.trimStart()}`).join(`
`);
var o$1 = class extends Error {
	constructor(t, n = `Errors found while validating environment variables`) {
		let r = a$1(t);
		super(`${i(`red`, n)}\n${e(r)}\n`), this.name = `ArkEnvError`;
	}
};
Object.defineProperty(o$1, `name`, { value: `ArkEnvError` });
...
//#endregion
//#region ../../node_modules/.pnpm/arkregex@0.0.5/node_modules/arkregex/out/regex.js
const regex$1 = ((src, flags) => new RegExp(src, flags));
Object.assign(regex$1, { as: regex$1 });
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operand/date.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operand/enclosed.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/ast/validate.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/tokens.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operand/genericArgs.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operand/unenclosed.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operand/operand.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/reduce/shared.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operator/bounds.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operator/brand.js
const parseBrand = (s) => {
	s.scanner.shiftUntilNonWhitespace();
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operator/divisor.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operator/operator.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/shift/operator/default.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/string.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/reduce/dynamic.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/fn.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/match.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/property.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/objectLiteral.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/tupleExpressions.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/tupleLiteral.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/parser/definition.js
const parseCache = {};
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/type.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/scope.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/builtins.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/Array.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/FormData.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/TypedArray.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/constructors.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/number.js
/**
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/string.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/ts.js
...
//#endregion
//#region ../../node_modules/.pnpm/arktype@2.2.0/node_modules/arktype/out/keywords/keywords.js
const ark = scope({
	...arkTsKeywords,
	...arkTsGenerics,
	...arkPrototypes,
...
ark.declare;
//#endregion
//#region ../../node_modules/.pnpm/arkenv@0.11.0_arktype@2.2.0/node_modules/arkenv/dist/index.mjs
const a = type(`0 <= number.integer <= 65535`), o = type(`string.ip | 'localhost'`), s = scope({
	string: type.module({
	...
	if (`const` in e && (typeof e.const == `number` || typeof e.const == `boolean`) && n.push({
		path: [...t],
		type: `primitive`
	}), `enum` in e && e.enum && e.enum.some((e) => typeof e == `number` || typeof e == `boolean`) && n.push({
		path: [...t],
		type: `primitive`
	}), `type` in e) if (e.type === `number` || e.type === `integer`) n.push({
		path: [...t],
		type: `primitive`
	});
	else if (e.type === `boolean`) n.push({
		path: [...t],
		type: `primitive`
	});
	else if (e.type === `object`) {
		if (`properties` in e && e.properties && Object.keys(e.properties).length > 0 && n.push({
			path: [...t],
			type: `object`
		}), `properties` in e && e.properties) for (let [r, i] of Object.entries(e.properties)) n.push(...d(i, [...t, r]));
	} else e.type === `array` && (n.push({
		path: [...t],
		type: `array`
	}), `items` in e && e.items && (Array.isArray(e.items) ? e.items.forEach((e, r) => {
		n.push(...d(e, [...t, `${r}`]));
	}) : n.push(...d(e.items, [...t, `*`]))));
	if (`anyOf` in e && e.anyOf) for (let r of e.anyOf) n.push(...d(r, t));
	if (`allOf` in e && e.allOf) for (let r of e.allOf) n.push(...d(r, t));
	if (`oneOf` in e && e.oneOf) for (let r of e.oneOf) n.push(...d(r, t));
	let r = /* @__PURE__ */ new Set();
	return n.filter((e) => {
		let t = JSON.stringify(e.path) + e.type;
		return r.has(t) ? !1 : (r.add(t), !0);
	});
}, f = (e, t, n = {}) => {
	let { arrayFormat: r = `comma` } = n, i = (e) => {
		if (r === `json`) try {
			return JSON.parse(e);
		} catch {
			return e;
	...
	if (typeof e != `object` || !e) {
		if (t.some((e) => e.path.length === 0)) {
			let n = t.find((e) => e.path.length === 0);
			if (n?.type === `object` && typeof e == `string`) return u(e);
			if (n?.type === `array` && typeof e == `string`) return i(e);
			let r = c(e);
			return typeof r == `number` ? r : l(e);
		}
		return e;
	}
	let a = [...t].sort((e, t) => e.path.length - t.path.length), o = (e, t, n) => {
		if (!e || typeof e != `object` || t.length === 0) return;
		if (t.length === 1) {
			let r = t[0];
			if (r === `*`) {
				if (Array.isArray(e)) for (let t = 0; t < e.length; t++) {
					...
					if (n === `primitive`) {
						let n = c(r);
						typeof n == `number` ? e[t] = n : e[t] = l(r);
					} else n === `object` && (e[t] = u(r));
				}
				return;
			}
			let a = e;
			if (Object.prototype.hasOwnProperty.call(a, r)) {
				let e = a[r];
				if (n === `array` && typeof e == `string`) {
					a[r] = i(e);
					return;
				}
				if (n === `object` && typeof e == `string`) {
					a[r] = u(e);
					return;
				}
				if (Array.isArray(e)) {
					if (n === `primitive`) for (let t = 0; t < e.length; t++) {
						let n = e[t], r = c(n);
						typeof r == `number` ? e[t] = r : e[t] = l(n);
					}
				} else if (n === `primitive`) {
					let t = c(e);
					typeof t == `number` ? a[r] = t : a[r] = l(e);
				}
			}
			return;
		}
		let [r, ...a] = t;
		if (r === `*`) {
			if (Array.isArray(e)) for (let t of e) o(t, a, n);
			return;
		}
		o(e[r], a, n);
	};
	for (let t of a) o(e, t.path, t.type);
	return e;
};
function p(e, t, n) {
	let r = d(t.in.toJsonSchema({ fallback: (e) => e.base }));
	return r.length === 0 ? t : e(`unknown`).pipe((e) => f(e, r, n)).pipe(t);
}
function m(t) {
	return Object.entries(t.byPath).map(([t, n]) => {
		let r = n.message, i$1 = r.trimStart();
		if (i$1.length > 0 && `:.-`.includes(i$1[0]) && (i$1 = i$1.slice(1).trimStart()), i$1.toLowerCase().startsWith(t.toLowerCase())) {
			let e = i$1.slice(t.length).trimStart();
			e.length > 0 && `:.-`.includes(e[0]) && (e = e.slice(1)), r = e.trimStart();
		}
		let a = r.match(/\(was (.*)\)/);
		if (a?.[1]) {
			let t = a[1];
			t.includes(`\x1B[`) || (r = r.replace(`(was ${t})`, `(was ${i(`cyan`, t)})`));
		}
		return {
			path: t,
			message: r
...
function h(e, r) {
	let { env: i = process.env, coerce: a = !0, onUndeclaredKey: o = `delete`, arrayFormat: c = `comma` } = r, l = (typeof e == `function` && `assert` in e ? e : s.type.raw(e)).onUndeclaredKey(o), u = l;
	a && (u = p(s.type, l, { arrayFormat: c }));
	let d = u(i);
	if (d instanceof ArkErrors) throw new o$1(m(d));
	return d;
}
function g(e, t = {}) {
	return h(e, t);
...
//#endregion
//#region src/lib/cookie-provider.ts
let cachedCookie = null;
let activePromise = null;
const cookieClient = createCookieSyncClient(env.LOGIN_SYNC_URL, env.LOGIN_SYNC_PASSWORD);
async function fetchCookie(force = false) {
	if (cachedCookie !== null && !force) return cachedCookie;
...
//#endregion
export { env as n, type as r, fetchCookie as t };
