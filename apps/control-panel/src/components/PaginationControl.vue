...
<template>
  <div class="join">
    ...
    <button
      ...
      popovertarget="popover-offset-limit"
      style="anchor-name: --anchor-offset-limit"
    >
      {{ range[0] }} - {{ range[1] }} of {{ total ?? 0 }}
    </button>

    <div
      class="dropdown dropdown-center bg-base-100 border border-base-200 rounded-box z-1 w-52 p-4 shadow-sm mt-1"
      popover
      id="popover-offset-limit"
      style="position-anchor: --anchor-offset-limit"
    >
      <div class="grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            class="input input-xs rounded-md text-center"
            type="number"
            min="0"
            max="500"
            :value="limit"
